...
export function simplifyElasticAppActions(input: string): string {
  // Using regex to match elasticAppAction tags that have type="file"
  const regex = /(<elasticAppAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/elasticAppAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<elasticAppAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</elasticAppAction>`;
    });

  return `<elasticAppArtifact id="code-content" title="Code Content" type="bundled">\n${fileContexts.join('\n')}\n</elasticAppArtifact>`;
}
