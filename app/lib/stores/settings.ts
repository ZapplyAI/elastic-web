...
const getInitialProviderSettings = (): ProviderSetting => {
  ...
  // Start with default settings - only enable Anthropic
  PROVIDER_LIST.forEach((provider) => {
    initialSettings[provider.name] = {
      ...provider,
      settings: {
        // Only enable Anthropic, disable all others
...
export const updateProviderSettings = (provider: string, settings: ProviderSetting) => {
  ...
  // Enforce Anthropic to always be enabled and other providers to be disabled
  if (provider === 'Anthropic') {
    updatedProvider.settings.enabled = true;
  } else {
    updatedProvider.settings.enabled = false;
  }
...
const getInitialTabConfiguration = (): TabWindowConfig => {
  ...
  try {
    const saved = localStorage.getItem('elasticApp_tab_configuration');
...
export const resetTabConfiguration = () => {
  ...
  localStorage.setItem('elasticApp_tab_configuration', JSON.stringify(defaultConfig));
};
