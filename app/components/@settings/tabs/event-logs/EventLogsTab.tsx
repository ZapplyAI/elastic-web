...
export function EventLogsTab() {
  ...
  const exportAsJSON = () => {
    try {
      ...
      if (typeof window !== 'undefined' && typeof document !== 'undefined') {
        ...
        a.href = url;
        a.download = `elasticApp-event-logs-${new Date().toISOString()}.json`;
  ...
  const exportAsCSV = () => {
    try {
      ...
      if (typeof window !== 'undefined' && typeof document !== 'undefined') {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `elasticApp-event-logs-${new Date().toISOString()}.csv`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add subtitle with elasticApp.diy
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('elasticApp.diy - AI Development Platform', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by elasticApp.diy', margin, doc.internal.pageSize.getHeight() - 10);
      ...
      // Save the PDF
      doc.save(`elasticApp-event-logs-${new Date().toISOString()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      if (typeof window !== 'undefined' && typeof document !== 'undefined') {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `elasticApp-event-logs-${new Date().toISOString()}.txt`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
